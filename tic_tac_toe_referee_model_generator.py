...
class TicTacToeReferee(object):
    ...
    NUM_EPOCHS=30
    def __init__(self, output_keys=OUTPUT_KEYS, epochs=NUM_EPOCHS, verbose=False):
        self.input_size = 9
        self.hidden_size = 15
    ...
    def import_training_data(self, filename="TicTacToeBoardClassifierData.csv", output_keys=OUTPUT_KEYS):
        ...
        with open(filename, newline='') as datafile:
            ...
            self.verbose_print(self.input_data)
            self.output_data = tf.constant(output_list, dtype=float)
            self.verbose_print(self.output_data)
    def train(self, learning_rate=1.0):
        self.model.compile(keras.optimizers.SGD(learning_rate=learning_rate), loss='mean_squared_error', metrics=['accuracy'])
        self.model.fit(self.input_data, self.output_data, batch_size=10, epochs=self.epochs, validation_split=0.001, shuffle=True, verbose=2)
...
def main() -> int:
    ...
    ap.add_argument("--learning-rate", type=float, default=1.0, help="The learning rate used for training.")
    ...
    if pa.action == "train":
        ...
        referee = TicTacToeReferee(output_keys=output_keys, epochs=pa.epochs, verbose=pa.verbose)
        referee.import_training_data(filename=pa.data_file)
        referee.train(pa.learning_rate)
        referee.save(pa.save_file)
    else:
        model = keras.models.load_model(pa.load_file)
        in_tensor = tf.constant([pa.input_board], dtype=float)
        print("Input:      " + str(in_tensor))
        output = model(in_tensor)
        if pa.verbose:
            print("Output Raw: " + str(output))
        print("Output:     " + str(tf.round(output)))
    return 0
